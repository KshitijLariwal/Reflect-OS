...
const regex = /const removeMessage = async \(entryId: string, messageIndex: number, e: React\.MouseEvent\) => \{[\s\S]*?\}\)\(\);\n    \}, 250\);\n  \};/;
...
if (code.match(regex)) {
  code = code.replace(regex, newRemove);
  fs.writeFileSync('src/App.tsx', code);
  console.log("Updated App.tsx removeMessage");
} else {
  console.log("Could not find removeMessage using regex");
}
